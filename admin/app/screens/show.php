...
<div style="width:100%;">
   ...
   <div style="float:left">
	...
	<h3>Status: 
		<span class="emph">
			<?php if($this->screen->is_connected()) { ?>
			 <?php if($this->screen->get_powerstate()) { ?>
	...
	<h3>Group: 
		<span class="emph">
			<?php $group = new Group($this->screen->group_id) ?>
					<a href="<?php echo ADMIN_URL.'/groups/show/'.$group->id ?>"><?php echo $group->name?></a>
		</span>
	</h3>
	<h3>Template: 
		<span class="emph">
			<?php $template = new Template($this->screen->template_id) ?>
					<a class="t-preview" href="<?php echo ADMIN_URL.'/templates/preview/'.$template->id.'?width=800' ?>"><?php echo $template->name?></a>
		</span>
	</h3>
<?php
if($this->canEdit && $this->screen->controls_display) {
?>
   <h3>Hours of Operation: <span class="emph"><?php echo $this->screen->time_on ?> - <?php echo $this->screen->time_off ?></span></h3>
<?php
}
?>
...
?>
	<h3>MAC: <span class="emph"><?php echo $mac?></span></h3>
	<h3>Last IP: <span class="emph"><?php echo $this->screen->last_ip?></span></h3>
<?php } ?>
   </div>
</div>
...
<div class="roundcont">
  ...
  <div class="roundcont_main">
    ...
    <ul>
	   ...
	   ?>
		  <li><a href="<?php echo ADMIN_URL.'/browse/show/'.$feed->id?>"><?php echo $feed->name?></a></li>
	   <?php
		  }
